...
( function( global, factory ) {
 ...
 simple = false;
		...
		p.nodeName = nodeName;
		p.children = children;
		p.attributes = null == attributes ? undefined : attributes;
		p.key = null == attributes ? undefined : attributes.key;
 ...
 ref.current = value;
	...
	function enqueueRender( component ) {
		if ( ! component._dirty && ( component._dirty = true ) && 1 == items.push( component ) ) {
			( defer )( rerender );
		}
	}
...
;
 ...
 node.style[i] = '';
				...
				for ( var i in value ) {
					node.style[i] = 'number' === typeof value[i] && false === IS_NON_DIMENSIONAL.test( i ) ? value[i] + 'px' : value[i];
				}
			}
 ...
 c.componentDidMount();
	...
	function diff( dom, vnode, context, mountAll, parent, componentRoot ) {
		if ( ! diffLevel++ ) {
			isSvgMode = null != parent && parent.ownerSVGElement !== undefined;
 ...
 dom.parentNode.replaceChild( out, dom );
		...
		if ( ! hydrating && vchildren && 1 === vchildren.length && 'string' === typeof vchildren[0] && null != fc && fc.splitText !== undefined && null == fc.nextSibling ) {
 ...
 return;
		...
		if ( 'undefined' === typeof component.constructor.getDerivedStateFromProps ) {
 ...
 component.prevProps = component.props;
		...
		if ( 0 !== renderMode ) {
			if ( 1 === renderMode || false !== options.syncComponentUpdates || ! component.base ) {
 ...
 return;
		...
		if ( isUpdate ) {
			...
			if ( 2 !== renderMode && component.shouldComponentUpdate && false === component.shouldComponentUpdate( props, state, context ) ) {
				skip = true;
...
params = {};
	...
	var IroComponent = /*@__PURE__*/( function( Component$$1 ) {
	  ...
	  if ( Component$$1 ) {
IroComponent.__proto__ = Component$$1;
}
	...
	function createArcPath( cx, cy, radius, startAngle, endAngle ) {
	  var largeArcFlag = 180 >= endAngle - startAngle ? 0 : 1;
	...
	var IroWheel = /*@__PURE__*/( function( IroComponent$$1 ) {
	  ...
	  if ( IroComponent$$1 ) {
IroWheel.__proto__ = IroComponent$$1;
}
	    ...
	    return (
	      h( 'svg', {
	        ...
	        h( 'g', { class: 'iro__wheel__hue', 'stroke-width': radius, fill: 'none' },
	          Array.apply( null, { length: 360 } ).map( function( _, hue ) {
 return (
	            h( 'path', {
	...
	function parseUnit( str, max ) {
	  var isPercentage = -1 < str.indexOf( '%' );
 ...
 this.set( value );
	...
	*/
	Color.rgbToHsv = function rgbToHsv( rgb ) {
	  ...
	  var hue;
	  var value = max;
	  var saturation = 0 === max ? 0 : delta / max;
	  switch ( max ) {
	    case min:
	      hue = 0; // achromatic
	      break;
	...
	*/
	Color.hsvToHsl = function hsvToHsl( hsv ) {
	  ...
	  var v = hsv.v / 100;
	  var l = ( 2 - s ) * v;
	  var divisor = 1 >= l ? l : ( 2 - l );

	  // Avoid division by zero when lightness is close to zero
	  var saturation = 1e-9 > divisor ? 0 : ( s * v ) / divisor;
	  return {
	    h: hsv.h,
	    s: saturation * 100,
	    l: l * 50
	...
	*/
	Color.hslToHsv = function hslToHsv( hsl ) {
	  var l = hsl.l * 2;
	  var s = ( hsl.s * ( ( 100 >= l ) ? l : 200 - l ) ) / 100;

	  // Avoid division by zero when l + s is near 0
	  var saturation = ( 1e-9 > l + s ) ? 0 : ( 2 * s ) / ( l + s );
	  return {
	    h: hsl.h,
	    s: saturation * 100,
	    v: ( l + s ) / 2
...
a = 1;
	...
	var IroSlider = /*@__PURE__*/( function( IroComponent$$1 ) {
	  ...
	  if ( IroComponent$$1 ) {
IroSlider.__proto__ = IroComponent$$1;
}
 ...
 return (
	...
	function createWidget( widgetComponent ) {
	  ...
	  var widgetFactory = function( parent, props ) {
	    ...
	    render(
	      h( widgetComponent, Object.assign( {}, {ref: function( ref ) {
 return widget = ref;
}},
	        props ) ),
	      widgetRoot
	...
	var ColorPicker = /*@__PURE__*/( function( Component$$1 ) {
	  ...
	  if ( Component$$1 ) {
ColorPicker.__proto__ = Component$$1;
}
...
args[ len ] = arguments[ len + 1 ];
	  ...
	  ColorPicker.prototype.updateColor = function updateColor( color, changes ) {
	    ...
	    if ( ! this._colorUpdateActive ) {
	      ...
	      if ( 'input' == this._colorUpdateSrc ) { // colorUpdateSrc is cleared in handeInput()
	      ...
	      h( 'div', {
	        ...
	        this.layout.map( function( ref ) {
	          ...
	          h( UiComponent, Object.assign( {},
	            state, options$$1, { onInput: function( type, hsv ) {
 return this$1.handleInput( type, hsv );
}, parent: this$1 } ) )
	        );
	    } )
	      )
...
pluginOptions = {};
	    ...
	    if ( ! ( -1 < installedPlugins.indexOf( plugin ) ) ) {
